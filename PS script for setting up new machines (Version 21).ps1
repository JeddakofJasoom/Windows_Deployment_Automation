...
# If the service is still running after 3 seconds, throw an error and STOP the script. 
		if ((Get-Service -Name wuauserv).Status -eq "Running") {
			Write-Host "WARNING: Windows updates failed to stop running. You will need to manually stop the service and process and rerun this script! ERROR: $_" -ForegroundColor Red
		throw "Windows Update service failed to stop within the timeout period."
			}
...
#region WINDOWS UPDATE SERVICE 

#create function to check current status for windows update service:
function Start-WindowsUpdate {
	
if ((Get-Service -name wuauserv).Status -eq "Running") {
	Write-Host "Windows Update service is currently RUNNING." -ForegroundColor Green Write-Host "Proceeding with Windows updates with autoreboot..." -ForegroundColor Yellow
} else {
	Write-Host "Windows Update service is currently NOT running." -ForegroundColor Red
	
		# Re-enable and start the Windows Update service
	Write-Host "Re-enabling Windows Update service..." -ForegroundColor Green
...
# ==========================
# Function Execution: Start-WindowsUpdate
# ==========================
Start-WindowsUpdate
...
# If after $maxRetries it still fails, handle the failure logging and reboot
    if (-not $success) {
        ...
        Write-Host "Windows Updates installed successfully. Rebooting in 5 seconds..." -ForegroundColor Green
    }
}
catch { 
