...
# Log file path (update this if needed)
$LogFile = "C:\Sources\initial_setup_log.txt"


# Start installation process
Write-Output "Starting Office 365 installation..."
Log-Message "Starting Office 365 installation."
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -or $LASTEXITCODE -eq $null) {
        ...
        Write-Output "Office 365 installation failed with exit code: $LASTEXITCODE"
    }
} catch {
    # Handle errors during the installation process
    $errorMessage = $_.Exception.Message
    Log-Message "Error during Office 365 installation: $errorMessage"
    Write-Output "Error during Office 365 installation: $errorMessage"
}
